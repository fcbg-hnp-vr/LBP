...
/// </remarks>
public class MyOptitrackSkeletonAnimator : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if ( skelState != null )
        {
            ...
            for ( int i = 0; i < m_skeletonDef.Bones.Count; ++i )
            {
                ...
                {
                    if (boneObject.name == SkeletonAssetName + "_Ab" /*boneId != 14 && boneId != 15 && boneId != 16 && boneId != 17 && boneId != 18 && boneId != 19 && boneId != 20 && boneId != 21*/)
                    {
                        Debug.Log(boneObject.name + " / " + boneId);

                        boneObject.transform.localPosition = bonePose.Position;

                        float angle = 180f * Time.deltaTime;
                        //transform.rotation *= Quaternion.AngleAxis(angle, Vector3.up);
                        boneObject.transform.localRotation = bonePose.Orientation * Quaternion.AngleAxis(angle, Vector3.right); //bonePose.Orientation ;
                    }
                    else
                    {
                        Debug.Log(boneObject.name + " / " + boneId);
